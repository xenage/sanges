...
use super::runtime::{
    build_guest_artifacts, guest_artifacts_stale, maybe_init_submodules, resolve_artifacts,
    write_guest_artifact_fingerprint,
};
use super::signing;
...
pub(super) fn build_distribution_binary(
    ...
    let path = build_native_binary(root, current, profile, refresh_guest, skip_guest_refresh)?;
    Ok(BuiltHostBinary {
...
fn build_native_binary(
    ...
    let _manifest_guard =
        prepare_embedded_assets(root, platform, profile, refresh_guest, skip_guest_refresh)?;
...
fn prepare_embedded_assets(
    ...
    if !skip_guest_refresh && (refresh_guest || guest_artifacts_stale(root, platform)?) {
        build_guest_artifacts(root, platform, profile)?;
        write_guest_artifact_fingerprint(root, platform)?;
    }
    let artifacts = resolve_artifacts(root, platform)?;
